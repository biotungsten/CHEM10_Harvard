...
class TestArduinoFunctions:
    ...
    def test_analog_read_write(self, confirm_physical_setup_present):
        # We check with differential set to 5, that should give a new read on each change since the changes are spaced sufficiently.
        diffs = []
        ...
        for _ in range(3):
            for value in [0, 128, 140, 255]:
                ...
                time.sleep(0.1)
                read_value = board.analog_read(self.ANALOG_READ_PIN, differential=5) 
                diff = abs(read_value - value)
                diffs.append(diff)
                assert diff < 10, f"Expected to read approximately {value} from pin {self.ANALOG_READ_PIN}, but got {read_value}."
        print(f"Analog read/write differences: {np.average(diffs)} ± {np.std(diffs)}")            

    def test_servo(self, confirm_physical_setup_present):
        board = confirm_physical_setup_present
        board.init_servo(self.SERVO_PIN)
